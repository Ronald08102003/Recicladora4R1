...
app.get('/api/ver-carrito', async (req, res) => {
    ...
    for (const id in carritoTemporal) {
        ...
        if (p.rows.length) items.push({ ...p.rows[0], cantidad: carritoTemporal[id], subtotal: p.rows[0].peso_kg * carritoTemporal[id] });
    }
    res.json({ items });
});
